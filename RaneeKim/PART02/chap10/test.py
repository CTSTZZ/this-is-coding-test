...
'''
1클래스 -> 10시간
2클래스 -> 3시간 (4번클래스를 먼저들어야함)  -> 온전한 수강시간은 5시간
3클래스 -> 2시간 (2번클래스를 먼저들어야함) -> 3시간(2클래스) + 3클래스(2시간) 
4클래스 -> 2시간 
5클래스 -> 1시간
...
'''
for i in range(n) :
    data = list(map(int,input().split())) 입력받은 애들  
